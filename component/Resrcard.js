...
export default function RestCard({restInfo}){
    ...
    return (
        <Link to={"/city/delhi/"+restInfo?.info?.id}>
               ...
               <div className="text-white font-bold relative bottom-7 pl-2 text-[20px] h-8 bg-linear-to-t from-red-950">{restInfo?.info?.aggregatedDiscountInfoV3?.header+" "+restInfo?.info?.aggregatedDiscountInfoV3?.subHeader}</div>
